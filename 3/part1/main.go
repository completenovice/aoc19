...
func getJourney(directions []string) ([]int, [][]int) {
	lastPoint := []int{0, 0}
	fmt.Printf("First plot: %+v\n", lastPoint)
	trackedPoints := [][]int{}
	for _, direction := range directions {
		for _, i := range getStint(lastPoint[0], lastPoint[1], direction) {
			trackedPoints = append(trackedPoints, i)
			lastPoint = []int{i[0], i[1]}
			// fmt.Printf("Last plotted point: %+v\n", lastPoint)
		}
	}
	fmt.Printf("Final plot: %+v\n", lastPoint)
	return lastPoint, trackedPoints
}

func calculateManhatten(in [][]int) []int {
	fmt.Println("Calculating Manhatten")
	distances := []int{}
	for _, i := range in {
		x := float64(i[0])
		y := float64(i[1])
		...
		fmt.Printf("%+v, %+v", x, y)
		distances = append(distances, int(x+y))
	}
	return distances
}
...
func main() {
	crossedPaths := [][]int{}
	_, journey1 := getJourney(directions1)
	_, journey2 := getJourney(directions2)
	for _, coord1 := range journey1 {
		for _, coord2 := range journey2 {
			if coord1[0] == coord2[0] {
				if coord1[1] == coord2[1] {
					fmt.Println("Match!")
					fmt.Printf("%+v\n", coord1)
					crossedPaths = append(crossedPaths, coord1)
				}
			}
		}
	}
	distances := calculateManhatten(crossedPaths)
	min := distances[0]
